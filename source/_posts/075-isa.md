...
那么位域是如何使用的呢？我们定义一个同样形式的结构体，用于对比两者的内存占用大小。
...
一个联合体占用的内存大小，是联合体中最宽元素的大小。Data 中的最宽元素是 d，占用 8 个字节内存大小。可以通过下面的代码验证：
...
本文创建的项目是 mac 项目，所以使用的是 __x86_64__ 架构。

如下所示，我们创建一个 RDPet 对象，然后让 5 个 RDPet 类型指针指向这个对象，此时该对象的引用计数是 5，然后分析一下 isa 是否如上述的那样。
...
在 Log 地方打上断点。在 debug area 使用 lldb 指令进行打印。
