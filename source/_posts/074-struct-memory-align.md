...
计算机中数据处理的最小单位是 Byte （字节），8 个二进制位称为一个字节。
...
你可能会想，A 和 B 都包含 int， char， short， long 四种类型。根据前面的基本数据类型大小依次进行求和，4 + 1 + 2 + 8 + 1 = 16，结果就是都打印 16 了。是这样的么？我们看看实际打印结果。
...
原则一的结果，不是我们前面打印的 24 和 32，所以又引出了内存对齐的第二个原则。
